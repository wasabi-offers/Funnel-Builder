import express from 'express';
import cors from 'cors';

const app = express();
app.use(cors());
app.use(express.json());

const REPLIT_TOKEN = 'eyJhbGciOiJSUzI1NiIsImtpZCI6IjB1a2x0QSJ9..lyAUObZN9rgzxmnk_z_2bDe920kCxhcPoC9kr6q5RxXhqn7c4r2r0fvcqWbO5d9uU0OfaswvWZE5RO6JxenRgW68KuIf2lspxe-NnaRBUFazuoM7j3BZ4y_PEnY6YRpsTe5NwD0tH4bL4x3LObcaWqO5jxP_ZssU6PyZNf8VoLloEmQo0ILm8RYAwO2sySFyT3wx3w5gK79xQI9IOJsBg5CxBhCzlbJ7pNgD6AT8EQhnHFIaT85BsFUR4BRP8yE_2Mysxsp9Qq6QUafrln2-55PD0OafQ7OJHY4NZOslHD_gPXVEMWgMCBRyh6vsKWpo4gUqv2PW_iLNskkRv7xddg';

// NOTA: Questo proxy NON funzionerà nell'ambiente Claude Code
// perché replit.com non è nella lista allowed_hosts.
// Usa la Supabase Edge Function invece!

// Proxy per GraphQL Replit
app.post('/api/replit', async (req, res) => {
  try {
    console.log('📤 Forwarding request to Replit GraphQL API');

    const response = await fetch('https://replit.com/graphql', {
      method: 'POST',
      headers: {
        'Content-Type': 'application/json',
        'Authorization': `Bearer ${REPLIT_TOKEN}`,
        'X-Requested-With': 'XMLHttpRequest',
        'User-Agent': 'Mozilla/5.0'
      },
      body: JSON.stringify(req.body)
    });

    const data = await response.json();

    console.log('✅ Response from Replit:', response.status);

    res.json(data);
  } catch (error) {
    console.error('❌ Proxy error:', error.message);
    res.status(500).json({
      error: error.message,
      hint: 'Questo proxy non funziona in Claude Code. Usa la Supabase Edge Function invece!'
    });
  }
});

const PORT = 3001;
app.listen(PORT, () => {
  console.log(`✅ Proxy server running on http://localhost:${PORT}`);
  console.log(`⚠️  ATTENZIONE: Questo proxy non funzionerà in Claude Code`);
  console.log(`   Usa la Supabase Edge Function invece!`);
});
